package com.elexyt.ugflweb.authentication.service;
...
public class LoginDetailsServiceImpl implements UserDetailsService{
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if(login != null && login.getIsActive()==1) {
            Set<GrantedAuthority> grantedAuthorities=login.getRoleSecurity().stream().map(roleSecurity -> new SimpleGrantedAuthority(roleSecurity.getRoleName())).collect(Collectors.toSet());
